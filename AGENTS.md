...
This fork dispatches only on `codex`, `zcode`, `qoder`, and `codebuddy`, and `bin/fm-spawn.sh` refuses every other adapter name at both its selection paths; the raw-launch escape hatch is never a supported harness.
Only `codex` carries upstream's live-verified adapter; `zcode`, `qoder`, and `codebuddy` are adapted from their verified CLI surfaces with pane-level live verification pending, are crewmate and scout only, and never run a secondmate.
If static `config/crew-harness` or `config/secondmate-harness` names a harness outside the fork's supported four, report it and fall back only to a supported adapter rather than launching it.
