...
test_no_profile_keeps_codex_profile_defaults() {
  local rec id out status expected launch
  id=profile-off-z1
  rec=$(make_spawn_case profile-off codex "$id")
  read_case_record "$rec"

  out=$(run_ship_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$LAUNCH_LOG" "$id" "$PROJ_DIR")
  status=$?
  expect_code 0 "$status" "codex spawn without profile flags should succeed"
  assert_contains "$out" "spawned $id harness=codex" "spawn did not report codex"
  assert_meta_profile "$HOME_DIR/state/$id.meta" codex default default

  launch=$(cat "$LAUNCH_LOG")
  expected="env -u CURSOR_AGENT -u CURSOR_INVOKED_AS -u GEMINI_CLI codex --dangerously-bypass-approvals-and-sandbox --disable hooks -c \"notify=[\\\"bash\\\",\\\"-c\\\",\\\"touch '$HOME_DIR/state/$id.turn-ended'\\\"]\" \"\$('${ROOT}/bin/fm-operational-input.sh' encode launch-brief < '$HOME_DIR/data/$id/launch-brief.md')\""
  [ "$launch" = "$expected" ] || fail "no-profile codex launch did not use the canonical launch kind"$'\n'"expected: $expected"$'\n'"actual:   $launch"
  pass "no --model/--effort records defaults and types the codex launch instructions"
}
...
test_relative_home_overrides_launch_with_absolute_cross_process_paths() {
  ...
  rec=$(make_spawn_case profile-relative-paths codex "$id")
  read_case_record "$rec"
  ...
  launch=$(cat "$LAUNCH_LOG")
  assert_contains "$launch" "touch '$home_real/state/$id.turn-ended'" \
    "relative FM_STATE_OVERRIDE leaked into codex's cross-process turn-end notify path"
...
test_home_defaults_preserve_absolute_or_resolve_relative_paths() {
  ...
  rec=$(make_spawn_case profile-home-defaults codex "$relative_id" "$absolute_id")
  read_case_record "$rec"
  home_real=$(cd "$HOME_DIR" && pwd -P)
  ...
  launch=$(cat "$LAUNCH_LOG")
  assert_contains "$launch" "touch '$home_real/state/$relative_id.turn-ended'" \
    "relative FM_HOME leaked into codex's default cross-process turn-end notify path"
  ...
  launch=$(cat "$LAUNCH_LOG")
  assert_contains "$launch" "touch '$home_real/state/$absolute_id.turn-ended'" \
    "absolute FM_HOME spelling changed in codex's default cross-process turn-end notify path"
...
test_absolute_override_spelling_is_preserved_in_launch_paths() {
  ...
  rec=$(make_spawn_case profile-absolute-paths codex "$id")
  read_case_record "$rec"
  home_real=$(cd "$HOME_DIR" && pwd -P)
  linked_home="$CASE_DIR/home-link"
  ln -s "$HOME_DIR" "$linked_home"
  ...
  launch=$(cat "$LAUNCH_LOG")
  assert_contains "$launch" "touch '$home_real/state/$id.turn-ended'" \
    "codex's cross-process turn-end notify path did not resolve through the real state directory"
...
test_unresolvable_relative_overrides_fail_loudly() {
  ...
  rec=$(make_spawn_case profile-unresolvable-paths codex "$id")
  read_case_record "$rec"
...
test_qoder_threads_model_and_reasoning_effort() {
  local rec id out status launch
  id=profile-qoder-z2
  rec=$(make_spawn_case profile-qoder qoder "$id")
  read_case_record "$rec"

  out=$(run_ship_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$LAUNCH_LOG" "$id" "$PROJ_DIR" --model glm-5 --effort high)
  status=$?
  expect_code 0 "$status" "qoder spawn with profile flags should succeed"
  assert_meta_profile "$HOME_DIR/state/$id.meta" qoder glm-5 high
  launch=$(cat "$LAUNCH_LOG")
  assert_contains "$launch" "qoder --model 'glm-5' --reasoning-effort 'high' --dangerously-skip-permissions" \
    "qoder launch did not thread model and reasoning-effort flags"
  assert_not_contains "$launch" "--tui-mode" "non-Pi launches must not receive Pi's TUI mode override"
  pass "qoder receives --model and --reasoning-effort profile flags"
}
...
test_qoder_omits_unverified_max_reasoning_effort() {
  local rec id out status launch
  id=profile-qoder-max-z6
  rec=$(make_spawn_case profile-qoder-max qoder "$id")
  read_case_record "$rec"

  out=$(run_ship_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$LAUNCH_LOG" "$id" "$PROJ_DIR" --model glm-5 --effort max)
  status=$?
  expect_code 0 "$status" "qoder spawn with unverified max reasoning effort should omit the effort flag"
  assert_meta_profile "$HOME_DIR/state/$id.meta" qoder glm-5 max
  launch=$(cat "$LAUNCH_LOG")
  assert_contains "$launch" "qoder --model 'glm-5' --dangerously-skip-permissions \"\$('${ROOT}/bin/fm-operational-input.sh' encode launch-brief < " \
    "qoder launch did not preserve the model flag and typed brief when max effort was omitted"
  assert_not_contains "$launch" "--reasoning-effort" "qoder launch must omit unverified max reasoning effort"
  assert_not_contains "$launch" "--effort" "qoder launch must not fall back to --effort for reasoning effort"
  pass "qoder omits unverified max reasoning effort"
}

test_qoder_omits_unverified_xhigh_reasoning_effort() {
  local rec id out status launch
  id=profile-qoder-xhigh-z6b
  rec=$(make_spawn_case profile-qoder-xhigh qoder "$id")
  read_case_record "$rec"

  # qoder's --help does not enumerate --reasoning-effort's accepted levels, so
  # only low, medium, and high are verified-passable; xhigh is omitted.
  out=$(run_ship_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$LAUNCH_LOG" "$id" "$PROJ_DIR" --model glm-5 --effort xhigh)
  status=$?
  expect_code 0 "$status" "qoder spawn with unverified xhigh reasoning effort should omit the effort flag"
  assert_meta_profile "$HOME_DIR/state/$id.meta" qoder glm-5 xhigh
  launch=$(cat "$LAUNCH_LOG")
  assert_contains "$launch" "qoder --model 'glm-5' --dangerously-skip-permissions \"\$('${ROOT}/bin/fm-operational-input.sh' encode launch-brief < " \
    "qoder launch did not preserve the model flag and typed brief when xhigh effort was omitted"
  assert_not_contains "$launch" "--reasoning-effort" "qoder launch must omit unverified xhigh reasoning effort"
  assert_not_contains "$launch" "--effort" "qoder launch must not fall back to --effort for reasoning effort"
  pass "qoder omits unverified xhigh reasoning effort"
}


test_codebuddy_threads_model_and_ignores_effort_axis() {
  local rec id out status launch
  id=profile-codebuddy-z7
  rec=$(make_spawn_case profile-codebuddy codebuddy "$id")
  read_case_record "$rec"

  out=$(run_ship_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$LAUNCH_LOG" "$id" "$PROJ_DIR" --model glm-5 --effort high)
  status=$?
  expect_code 0 "$status" "codebuddy spawn with model and ignored effort should succeed"
  assert_meta_profile "$HOME_DIR/state/$id.meta" codebuddy glm-5 high
  launch=$(cat "$LAUNCH_LOG")
  assert_contains "$launch" "codebuddy --model 'glm-5' -y" \
    "codebuddy launch did not thread model"
  assert_not_contains "$launch" "--effort" "codebuddy launch must not pass unsupported --effort"
  assert_not_contains "$launch" "--reasoning-effort" "codebuddy launch must not pass an unverified reasoning-effort flag"
  assert_not_contains "$launch" "--thinking" "codebuddy launch must not pass pi thinking flag"
  pass "codebuddy receives --model and omits the unsupported effort axis"
}
...
# config/claude-permission-mode (bin/fm-spawn.sh header): an unrecognized token
# refuses every spawn from the home before endpoint or metadata, whichever
# supported harness it would launch.
test_claude_permission_mode_invalid_refuses_before_endpoint_or_metadata() {
  local rec id out status
  id=permmode-invalid-z22
  rec=$(make_spawn_case permmode-invalid codex "$id")
  read_case_record "$rec"
