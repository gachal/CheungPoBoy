...
make_toolchain() {
  local dir=$1 fakebin
  fakebin=$(fm_fakebin "$dir")
  fm_fake_exit0 "$fakebin" node chrome-devtools-axi codex
...
test_sweep_respawns_authoritatively_missing_pi_secondmate() {
  ...
  assert_contains "$(cat "$log")" "new-window" "an authoritatively missing codex secondmate should be relaunched"
  assert_not_contains "$(cat "$log")" "kill-window" "an absent window should not need a destructive pre-kill"
  pass "sweep: an authoritatively missing codex secondmate window is relaunched"
}
