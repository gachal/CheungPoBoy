...
test_spawn_preserves_orca_metadata_when_pathless_worktree_cleanup_fails() {
  ...
  out=$( HOME="$SPAWN_HOME" CLAUDE_CONFIG_DIR='' PATH="$FB:$PATH" FM_ORCA_LOG="$LOG" FM_ORCA_RESPONSES="$RESP" \
    ...
    "$ROOT/bin/fm-spawn.sh" "$id" "$proj" codex --mode no-mistakes --yolo off --backend orca 2>&1 )
  status=$?
...
test_spawn_writes_orca_metadata_and_launches_harness() {
  ...
  assert_contains "$out" "spawned $id harness=codex kind=ship mode=no-mistakes yolo=off window=fm-$id worktree=$wt" \
  ...
  assert_contains "$(cat "$log")" "codex --dangerously-bypass-approvals-and-sandbox --disable hooks" \
...
test_spawn_refuses_orca_secondmate_before_home_mutation() {
  ...
  out=$( FM_ROOT_OVERRIDE="$ROOT" FM_HOME="$home" FM_STATE_OVERRIDE="$state" FM_DATA_OVERRIDE="$data" FM_CONFIG_OVERRIDE="$config" \
    ...
    "$ROOT/bin/fm-spawn.sh" "$id" "$subhome" codex --backend orca --secondmate 2>&1 )
  status=$?
  set +e
...
test_peek_send_and_crew_state_route_through_orca_meta() {
  ...
  fm_write_meta "$state/$id.meta" \
    "window=fm-$id" "endpoint_task_id=$id" "terminal=term-io" "worktree=$wt" "project=$wt" "harness=codex" "kind=scout" "backend=orca"
  touch "$state/.last-watcher-beat"
  orca_case io-path
...
test_peek_and_crew_state_fail_closed_on_orca_error_json() {
  ...
  fm_write_meta "$state/$id.meta" \
    "window=fm-$id" "endpoint_task_id=$id" "terminal=term-stale" "worktree=$wt" "project=$wt" "harness=codex" "kind=scout" "backend=orca"
  touch "$state/.last-watcher-beat"
...
test_teardown_refuses_orca_missing_worktree_id() {
  ...
  fm_write_meta "$state/$id.meta" \
    ...
    "harness=codex" "kind=scout" "mode=no-mistakes" "yolo=off" "backend=orca" \
    "decisions_reviewed=1" "decision_keys="
