...
add_ship_task() {
  local dir=$1 id=$2 harness=${3:-codex}
...
test_disabled_relaunch_clears_prior_trace_context() {
  ...
  grep -q '^unset TRACEPARENT; .*codex' "$dir/fake/literal" \
...
test_relaunch_requires_a_note_for_a_ship_task() {
  ...
  [ "$(cat "$dir/fake/command")" = codex ] || fail "a refused relaunch must not stop the agent"
...
test_same_harness_relaunch_keeps_identity_and_reuses_the_endpoint() {
  ...
  assert_contains "$out" "relaunched rl1 harness=codex from=codex" "the outcome should name the transition"
  ...
  [ "$gen_after" != "$gen_before" ] \
    || fail "a relaunch must not keep the pre-relaunch busy generation, got '$gen_after'"
  ...
  pass "fm-control relaunch: same harness keeps identity and reuses the endpoint"
}

test_harness_switch_moves_the_record_and_clears_prior_wiring() {
  local dir out rc
  dir=$(new_case switch rl4)
  add_ship_task "$dir" rl4 codebuddy
  printf 'codex' > "$dir/fake/becomes"
  out=$(run_control "$dir" rl4 relaunch --harness codex --note "switching runtime"); rc=$?
  expect_code 0 "$rc" "a harness switch should succeed"$'\n'"$out"
  assert_contains "$out" "harness=codex from=codebuddy" "the outcome should name both harnesses"
  ...
  [ "$(journal_field "$dir" rl4 from_harness)" = codebuddy ] || fail "the journal should record the origin harness"
...
test_harness_switch_does_not_carry_the_old_profile_axes() {
  ...
  printf 'codebuddy' > "$dir/fake/becomes"
  out=$(run_control "$dir" rl5 relaunch --harness codebuddy --note "switching runtime"); rc=$?
  expect_code 0 "$rc" "a harness switch should succeed"$'\n'"$out"
...
test_harness_switch_resolves_a_prefixed_recorded_harness() {
  ...
  add_ship_task "$dir" rl32 zcode-2
  printf 'zcode-2' > "$dir/fake/command"

  out=$(run_control "$dir" rl32 relaunch --harness codex --note "switching runtime"); rc=$?
  expect_code 0 "$rc" "relaunch should resolve a prefixed recorded harness"$'\n'"$out"
  [ "$(sed -n '1p' "$dir/fake/literal")" = /exit ] \
    || fail "relaunch should stop a zcode-prefixed task with zcode's exit command"
  [ "$(meta_field "$dir" rl32 harness)" = codex ] \
    || fail "relaunch should publish the explicitly selected replacement harness"
  [ "$(journal_field "$dir" rl32 from_harness)" = zcode-2 ] \
    || fail "relaunch should retain the recorded harness basename in its provenance"
  assert_contains "$out" "harness=codex from=zcode-2" \
...
test_prefixed_recorded_harness_requires_explicit_replacement() {
  ...
  add_ship_task "$dir" rl34 zcode-2
  printf 'zcode-2' > "$dir/fake/command"
  ...
  assert_contains "$out" "would substitute the canonical adapter 'zcode'" \
...
test_secondmate_relaunch_onto_a_crewmate_only_adapter_refuses_before_stop() {
  ...
  printf '%s' "$dir/smhome" > "$dir/fake/cwd"
  out=$(run_control "$dir" sm7 relaunch --harness zcode); rc=$?
  ...
  [ "$(meta_field "$dir" sm7 harness)" = codex ] \
...
test_spawn_relaunch_without_a_harness_reuses_the_recorded_one() {
  ...
  add_ship_task "$dir" rl21 codex
  mkdir -p "$dir/home/config"
  printf 'qoder\n' > "$dir/home/config/crew-harness"
  printf 'zsh' > "$dir/fake/command"
  out=$(run_spawn "$dir" rl21 --relaunch)
  [ "$(meta_field "$dir" rl21 harness)" = codex ] \
    || fail "fm-spawn --relaunch without --harness must reuse the recorded harness, got '$(meta_field "$dir" rl21 harness)'"
  assert_contains "$out" "spawned rl21 harness=codex" "the launch should report the recorded harness"
...
test_checkpoint_refuses_uninspectable_head_and_status() {
  ...
  [ "$(cat "$dir/fake/command")" = codex ] || fail "HEAD inspection failure must not stop the agent"

  dir=$(new_case badstatus rl23)
  add_ship_task "$dir" rl23 codex
  make_git_failure_stub "$dir"
  ...
  [ "$(cat "$dir/fake/command")" = codex ] || fail "status inspection failure must not stop the agent"
...
test_prepublication_abort_retires_replacement_wiring_and_busy_state() {
  ...
  [ "$(meta_field "$dir" rl28 harness)" = codex ] \
...
test_secondmate_checkpoint_refuses_unreadable_child_state() {
  ...
  [ "$(cat "$dir/fake/command")" = codex ] || fail "child record failure must not stop the secondmate"
...
SH
  ...
  [ "$(cat "$dir/fake/command")" = codex ] || fail "child traversal failure must not stop the secondmate"
...
test_direct_spawn_relaunch_participates_in_the_lifecycle_lock() {
  ...
  out=$(run_spawn "$dir" rl26 --relaunch --harness codex); rc=$?
  kill "$holder" 2>/dev/null || true
  wait "$holder" 2>/dev/null || true
...
test_spawn_relaunch_refuses_a_live_agent() {
  ...
  add_ship_task "$dir" rl15 codex
  out=$(run_spawn "$dir" rl15 --relaunch --harness codex); rc=$?
...
SH
  ...
  out=$(run_spawn "$dir" rl37 --relaunch --harness codex); rc=$?
...
SH
  ...
  out=$(run_spawn "$dir" rl38 --relaunch --harness codex); rc=$?
...
test_spawn_relaunch_refuses_a_pending_authoritative_close() {
  ...
  out=$(run_spawn "$dir" rl36 --relaunch --harness codex); rc=$?
...
test_spawn_relaunch_refuses_a_pane_outside_the_worktree() {
  ...
  add_ship_task "$dir" rl18 codex
  printf 'zsh' > "$dir/fake/command"
  printf '%s' "$dir/proj" > "$dir/fake/cwd"
  out=$(run_spawn "$dir" rl18 --relaunch --harness codex); rc=$?
